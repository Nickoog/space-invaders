...
describe('getIdsForLevel', () => {
  it('returns the correct number of IDs', () => {
    expect(getIdsForLevel(1)).toHaveLength(TOTAL);
  });

  it('level 1 starts at Pokémon ID 1', () => {
    expect(getIdsForLevel(1)[0]).toBe(1);
  });

  it('level 1 ends at Pokémon ID 55', () => {
    expect(getIdsForLevel(1).at(-1)).toBe(TOTAL);
  });

  it('level 2 continues immediately after level 1', () => {
    const last1 = getIdsForLevel(1).at(-1);
    const first2 = getIdsForLevel(2)[0];
  ...
  it('produces different IDs for different levels', () => {
    const ids1 = getIdsForLevel(1);
    const ids2 = getIdsForLevel(2);
  ...
  it('cycles back to the same IDs after GEN1_COUNT Pokémon', () => {
    ...
    const ids1 = getIdsForLevel(1);
    const ids152 = getIdsForLevel(1 + GEN1_COUNT); // 152nd level restarts the cycle
    expect(ids152).toEqual(ids1);
  });
});

// Helper — avoids inline magic numbers in tests
function getIdesForLevel3() {
  return getIdsForLevel(3);
}
